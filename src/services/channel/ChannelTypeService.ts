...
const handleChannelOffer = async (
    ...
    let channelIndex: Array<Channel> = await getLatestOnlineHostChannel({ channelId }) as Array<Channel>;
...
const handleChannelStatus = async (
    ...
    let channelIndex: Array<Channel> = await getLatestOnlineHostChannel({ channelId, channelStatus:"STARTED" }) as Array<Channel>;
    const channel = channelIndex.find(value => value.channelType.toLowerCase() == channelType.toLowerCase());
	if (!channel?.channelId) {
		throw {
			statusCode: 404,
...
const handleChannelApproxTime = async (
    ...
    astrologerData.currentChannel[channelType.toLowerCase() as keyof AstrologerCurrentChannel] = {
        enabled: true,
        approxTime: value, 
    }
    const response = await updateChannelApproxTime({channelId, createTs: channelData.createTs, currentChannel: astrologerData.currentChannel, channelType: channelData.channelType, value});
    return response;
}
