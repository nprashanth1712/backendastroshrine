...
const handleDisableChannel = async ({channelId, channelType}: {channelId: string, channelType: string}) => {
    ...
    const channelKey = `${channel?.channelType.toLowerCase()}` as keyof UserWaitlist;
    hostUserWaitlist[channelKey] = channel?.waitlist;
    hostCurrentChannel[channelKey] = {...hostCurrentChannel[channelKey], approxTime: 0, enabled: false}
    ...
    handleDisableChannelPostProcess({...channel});
    console.timeEnd();
