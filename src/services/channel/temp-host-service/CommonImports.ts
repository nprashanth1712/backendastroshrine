...
const fetchActiveChannelByIdAndType = async ({channelId, channelType}: {channelId: string, channelType: string}) => {
    const channelIndex: Array<Channel> = await getLatestOnlineHostChannel({
            channelId,
        });
    const currentChannel = channelIndex.find((value) => value.channelType.toLowerCase() == channelType.toLowerCase());
