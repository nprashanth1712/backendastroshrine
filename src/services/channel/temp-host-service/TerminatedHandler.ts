...
export const callTerminatedHandler = async ({
	...
	let totalTimeSpent: NonNullable<number> =
		channelDataModified.tempHost.endTime! - channelDataModified.tempHost.startTime!;
	...
	// users amount used
	let amountUsed: NonNullable<number> = (totalTimeSpent / 60000) * channelDataModified.rate;
	amountUsed = amountUsed - (amountUsed / 100) * channelDataModified.offer;

	// updated users data
	let userAmountAvailable: NonNullable<number> = userData.balance - amountUsed;
	const updatedUserBalance = userAmountAvailable < 0 ? 0 : userAmountAvailable;

	// update host time spent
	...
	const transactItemsData: TerminateCallDataToUpdate = {
		...
		userOrderData: {
			ts: channelDataModified.tempHost.startTime as number,
			status: "SUCCESS",
			amount: amountUsed,
		},
