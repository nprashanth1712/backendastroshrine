...
const getHostUserFollowDataRouter = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (!userId) {
			throw {statusCode: 400, code: "InvalidParameter", message: invalidParameter("userId")};
            return; 
        }
        if (!id) {
...
const addHostConnectionRouter = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const updatedFollowers = await updateAstrologerHostProfile({id, name: hostData.name, hostProfile: {...hostData.hostProfile, followers: hostData.hostProfile.followers+1}})
