...
class CognitoExpress {
    ...
    hasFinishedProcessing: Promise<unknown> | undefined;
    pems: { [key: string]: any } = {};
    constructor(config: {
    ...
    async validate(
        token: string,
        callback: (arg0: any, arg1: string | jwt.JwtPayload | null) => void
    ) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if ((decodedJwt.payload as JwtPayload).iss !== this.iss) {
                    throw new TypeError("token is not from your User Pool");
                }

                if (
                    (decodedJwt.payload as JwtPayload).token_use !==
                    this.tokenUse
                )
                    throw new TypeError(`Not an ${this.tokenUse} token`);

                let kid: any = decodedJwt.header.kid;
